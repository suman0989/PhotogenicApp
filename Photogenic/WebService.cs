	...
	namespace Photogenic
	{
		...
		public bool signUpUser(string email, string fname, string lname, string password){
			try{
				using (HttpClient client = new HttpClient()){
					client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
					var postData = new List<KeyValuePair<string, string>>();
					    postData.Add(new KeyValuePair<string, string>("email", email));
						postData.Add(new KeyValuePair<string, string>("first_name", fname));
						postData.Add(new KeyValuePair<string, string>("last_name", lname));
						//postData.Add(new KeyValuePair<string, string>("mobile", password));
						postData.Add(new KeyValuePair<string, string>("pass", password));
						postData.Add(new KeyValuePair<string, string>("appKey", PhotogenicConfiguration.APP_KEY));

					var content = new System.Net.Http.FormUrlEncodedContent(postData);
					var url = PhotogenicConfiguration.API_URL + "/user_regs";
					var result = client.PostAsync(url, content).Result;
					if (result.IsSuccessStatusCode)
					{
						try{
							...
							if (contentData.data.Count > 0)
							{
								...
								if (user.email == null || user.email == string.Empty){
									...
									Debug.WriteLine("Signup Success: {0}", user.first_name + user.last_name);
									//share login data ....
									SharedVariables.Default.curreentUser = user;
									return true;
								}
